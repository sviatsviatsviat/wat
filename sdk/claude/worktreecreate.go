...
func (c *chain) WorktreeCreate(fn func(context.Context, run.Hook[WorktreeCreate], WorktreeCreateResults) (WorktreeCreateOutput, error)) *chain {
	...
	c.reg.RegisterHandler(Dialect, run.Handler(func(ctx context.Context, hook run.Hook[WorktreeCreate]) (WorktreeCreateOutput, error) {
		return fn(ctx, hook, worktreeCreateResults{})
	}))
	return c
}
