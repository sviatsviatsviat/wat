...
func TestDecodeEncode_StopFollowUp(t *testing.T) {
	...
	if stop.ModelID != "claude-opus-4-7" {
		t.Fatalf("ModelID = %q", stop.ModelID)
	}
	if len(stop.ModelParams) != 2 || stop.ModelParams[0].ID != "thinking" || stop.ModelParams[0].Value != "true" {
		t.Fatalf("ModelParams = %+v", stop.ModelParams)
	}
