...
// PreToolUseResults.Ask encodes permission "ask", but Cursor does not enforce
// ask for preToolUse today; prefer Allow or Deny. Handler errors exit 1 under
// Cursor's default fail-open policy. Hook stdout JSON is encoded internally
// from Output (sealed; only this package implements it). Payloads must include
