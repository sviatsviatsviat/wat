...
func TestParseClaude_mappableAndExtras(t *testing.T) {
	cfg, _, err := Parse([]byte(claudeSettings), agnostic.Claude)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseCopilot_timeoutAlias(t *testing.T) {
	...
	cfg, _, err := Parse([]byte(raw), agnostic.Copilot)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseCopilot_powershellOnlyNotCommand(t *testing.T) {
	...
	cfg, warns, err := Parse([]byte(raw), agnostic.Copilot)
	if err != nil {
		t.Fatal(err)
	}
	if len(warns) == 0 {
	...
	out, _, err := Emit(cfg, agnostic.Copilot)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseCursor_dedicatedEvent(t *testing.T) {
	cfg, _, err := Parse([]byte(cursorDedicatedSettings), agnostic.Cursor)
	if err != nil {
		t.Fatal(err)
	}
	pre := cfg.Hooks[agnostic.KindPreTool]
...
func TestEmitClaude_roundTrip(t *testing.T) {
	cfg1, _, err := Parse([]byte(claudeSettings), agnostic.Claude)
	if err != nil {
		t.Fatal(err)
	}
	...
	pre[0].TimeoutSec = 99
	out, _, err := Emit(cfg1, agnostic.Claude)
	if err != nil {
		t.Fatal(err)
	}
	cfg2, _, err := Parse(out, agnostic.Claude)
	if err != nil {
		t.Fatal(err)
	}
	...
	cfg1.Hooks[agnostic.KindPreTool][0].Raw = cfg2.Hooks[agnostic.KindPreTool][0].Raw
	assertClaudeConfigEqual(t, cfg1, cfg2)
}

func TestEmitCopilot_roundTrip(t *testing.T) {
	cfg1, _, err := Parse([]byte(copilotSettings), agnostic.Copilot)
	if err != nil {
		t.Fatal(err)
	}
	pre := cfg1.Hooks[agnostic.KindPreTool]
	...
	out, _, err := Emit(cfg1, agnostic.Copilot)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEmitCursor_roundTrip(t *testing.T) {
	cfg1, _, err := Parse([]byte(cursorSettings), agnostic.Cursor)
	if err != nil {
		t.Fatal(err)
	}
	out, _, err := Emit(cfg1, agnostic.Cursor)
	if err != nil {
		t.Fatal(err)
	}
	cfg2, _, err := Parse(out, agnostic.Cursor)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEmitCursor_dedicatedEventRoundTrip(t *testing.T) {
	cfg1, _, err := Parse([]byte(cursorDedicatedSettings), agnostic.Cursor)
	if err != nil {
		t.Fatal(err)
	}
	out, _, err := Emit(cfg1, agnostic.Cursor)
...
func TestClaude_handlerExtraFieldsRoundTrip(t *testing.T) {
	...
	cfg1, _, err := Parse([]byte(raw), agnostic.Claude)
	if err != nil {
		t.Fatal(err)
	}
...
func TestClaude_claudeOnlyHandlerPreservesMatcher(t *testing.T) {
	...
	cfg1, warns, err := Parse([]byte(raw), agnostic.Claude)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCopilot_handlerExtraFieldsRoundTrip(t *testing.T) {
	...
	cfg1, _, err := Parse([]byte(raw), agnostic.Copilot)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCursor_handlerExtraFieldsRoundTrip(t *testing.T) {
	...
	cfg1, _, err := Parse([]byte(raw), agnostic.Cursor)
	if err != nil {
		t.Fatal(err)
	}
...
func assertEntriesByKind(t *testing.T, a, b map[agnostic.Kind][]Entry) {
	t.Helper()
	if len(a) != len(b) {
