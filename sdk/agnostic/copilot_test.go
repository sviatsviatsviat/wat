...
func TestCopilotDecodeEncode_PreToolDeny(t *testing.T) {
	...
	if ev.Kind != KindPreTool || ev.Session != "s1" || ev.Cwd != "/w" {
		t.Fatalf("bad event: %+v", ev)
	}
	if ev.Tool == nil || ev.Tool.Name != ToolBash || ev.Tool.Native != "bash" || ev.Tool.Shell != "rm -rf /" {
...
func TestCopilotDecode_VSCodeStop(t *testing.T) {
	...
	if ev.Kind != KindStop || ev.Name != "Stop" {
...
func TestCopilotDecode_VSCodePreToolBash(t *testing.T) {
  ...
  "tool_input": {"command": "ls -la"}
	...
	if ev.Tool == nil || ev.Tool.Name != ToolBash || ev.Tool.Native != "Bash" || ev.Tool.Shell != "ls -la" {
...
func TestCopilotDecode_NotificationWithoutEventHint(t *testing.T) {
  ...
  "notification_type": "shell_completed"
	...
	if ev.Kind != KindNotification || ev.Name != "Notification" {
		t.Fatalf("Kind=%v Name=%q", ev.Kind, ev.Name)
	}
...
func TestCopilotDecode_InvalidJSON(t *testing.T) {
	...
	if !errors.Is(err, copilot.ErrUnrecognizedFormat) {
...
func TestCopilotEncode_PreToolAllowModifiedArgs(t *testing.T) {
	c := &CopilotCodec{}
	ev := &Event{Agent: Copilot, Kind: KindPreTool, Name: "preToolUse"}
...
func TestCopilotEncode_StopFollowUp(t *testing.T) {
	c := &CopilotCodec{}
	ev := &Event{Agent: Copilot, Kind: KindStop, Name: "agentStop"}
	out, code, err := c.Encode(ev, Result{FollowUp: "run the tests"})
	if err != nil || code != 0 {
		t.Fatal(err, code)
	}
...
func TestCopilotEncode_SubagentStopFollowUp(t *testing.T) {
	c := &CopilotCodec{}
	ev := &Event{Agent: Copilot, Kind: KindSubagentStop, Name: "subagentStop"}
	out, code, err := c.Encode(ev, Result{FollowUp: "finish review"})
	if err != nil || code != 0 {
		t.Fatal(err, code)
	}
...
func TestCopilotEncode_PermissionRequestDenyInterrupt(t *testing.T) {
	...
	out, code, err := c.Encode(ev, Result{Decision: DecisionDeny, Reason: "blocked", HaltSession: true})
	if err != nil {
		t.Fatal(err)
	}
	if code != CopilotWarnExit {
		t.Fatalf("code=%d, want %d", code, CopilotWarnExit)
	}
...
func TestCopilotEncode_PostToolFailureContext(t *testing.T) {
	...
	out, code, err := c.Encode(ev, Context("retry with smaller input"))
	if err != nil {
		t.Fatal(err)
	}
	if code != CopilotWarnExit {
...
func TestCopilotEncode_SessionStartContext(t *testing.T) {
	c := &CopilotCodec{}
	ev := &Event{Agent: Copilot, Kind: KindSessionStart, Name: "sessionStart"}
	out, code, err := c.Encode(ev, Context("project uses go test ./..."))
	if err != nil || code != 0 {
		t.Fatal(err, code)
	}
...
func TestCodecFor_Copilot(t *testing.T) {
	codec, err := CodecFor(Copilot)
	if err != nil {
		t.Fatal(err)
	}
	if codec.Dialect() != Copilot {
		t.Fatalf("Dialect() = %v, want Copilot", codec.Dialect())
	}
}
