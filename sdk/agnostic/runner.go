...
func (m *Mux) Serve(ctx context.Context, in io.Reader, out io.Writer, errw io.Writer, opts ...Option) int {
	...
	if err != nil {
		_, _ = fmt.Fprintf(errw, "agnostic: read stdin: %v\n", err)
		return 1
	}
	if len(payload) == 0 {
		_, _ = fmt.Fprintln(errw, "agnostic: empty stdin")
		return 1
	}
	...
	if dialect == Unknown {
		_, _ = fmt.Fprintln(errw, "agnostic: unknown dialect")
		return 1
	}

	codec, err := CodecFor(dialect)
	if err != nil {
		_, _ = fmt.Fprintf(errw, "agnostic: %v\n", err)
		return 1
	}
	...
	if err != nil {
		_, _ = fmt.Fprintf(errw, "agnostic: decode: %v\n", err)
		return 1
	}
	...
	if err != nil {
		_, _ = fmt.Fprintf(errw, "agnostic: encode: %v\n", err)
		return 1
	}
	if len(stdout) > 0 {
		if _, err := out.Write(stdout); err != nil {
			_, _ = fmt.Fprintf(errw, "agnostic: write stdout: %v\n", err)
			return 1
		}
	}
	return code
