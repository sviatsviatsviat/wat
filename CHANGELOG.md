...
- Agnostic `On*` registration fans adapter handlers onto each agent SDK `Adapter()` Chain (native Decode/Encode owned by `sdk/claude`, `sdk/copilot`, `sdk/cursor`); inbound `MapEvent` maps native payloads to unified events
...
- `run.Invocation` and hook wrappers: agnostic kind types (`PreToolHook`, …) and per-agent generic `Hook[E]` / `AnyHook`, carrying typed events plus serve-time settings (`Dialect`, `EventHint`, `Getenv`, `DialectConfig`) and `Raw()` for untouched native JSON; unified handler signatures `(ctx, hook, results)` for result hooks and `(ctx, hook) error` for observe hooks across all four SDKs
