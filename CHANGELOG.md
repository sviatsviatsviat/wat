...
- `wat test --fixture` runs the user's hook script against a fixture JSON payload (file or stdin `-`); prints fixture agent/event, hook stdout JSON, decision when present, and exit code; optional `--verbose` for hook stderr; sample fixtures under `testdata/fixtures/`
...
- Unified `agnostic.Event` / `Kind` types with normalized `ToolCall.Name` values for hook authors; `ToolCall.Input` is a typed `tools.Input` (`sdk/agnostic/tools`) with `AsBash`, `AsWrite`, `AsEdit`, `AsRead`, and related accessors (path/`file_path` aliases)
...
- Hook output and portable result types are sealed interfaces in `sdk/agnostic` constructed only via Chain/`On*`-injected `*Results` builders (and `With*`); `nil` is the no-op; Results wrappers hold native agent outputs (no parallel Encode/`Build*` path)
- Agnostic `On*` registration fans adapter handlers onto each agent SDK `Adapter()` Chain (native Decode/Encode owned by `sdk/claude`, `sdk/copilot`, `sdk/cursor`); unexported inbound mappers beside each `On*` convert native payloads to unified events
