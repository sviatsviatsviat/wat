...
- `sdk/run` process lifecycle with `Main`; dialect codec, handler bag, process router, and `Handler` / `ObserveHandler` live in `internal/hookkit`; agent and agnostic SDKs register via zero-arg `UseHooks`; `Main` auto-detects one dialect, peeks the event name, then decodes the payload once before dispatching typed registrations
- Agnostic `UseHooks` registration fans adapter handlers onto each agent SDK via `UseHooks()` (native encode owned by `sdk/claude`, `sdk/copilot`, `sdk/cursor`; decode is internal to each SDK and `sdk/run`); inbound mappers in `sdk/agnostic/internal/{claude,cursor,copilot}` convert native payloads to unified events
...
- Typed `UseHooks().OnPreTool`, `OnPostTool`, `OnStop`, and related handlers plus observe-only `OnSessionEnd` with fluent chaining; hook scripts call `run.Main` (auto-detects dialect)
