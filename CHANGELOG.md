...
- Hook output and portable result types are sealed interfaces constructed only via Chain/`On*`-injected `*Results` builders (and `With*`); `nil` is the no-op; dialect codecs use package `Build*` helpers without exposing Results construction
