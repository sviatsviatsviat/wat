...
- Agnostic `On*` registration fans adapter handlers onto each agent SDK `Adapter()` Chain (native Decode/Encode owned by `sdk/claude`, `sdk/copilot`, `sdk/cursor`); inbound mappers in `sdk/agnostic/internal/{claude,cursor,copilot}` convert native payloads to unified events
