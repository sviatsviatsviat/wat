...
- `sdk/run` process lifecycle with `Main`, `Hook` / `Invocation` / `Config` (aliases), and options; dialect codec, handler bag, process router, and `Handler` / `ObserveHandler` live in `internal/hookkit`; agent and agnostic SDKs register via zero-arg `UseHooks`; `Main` selects one dialect, peeks the event name, then decodes the payload once before dispatching typed registrations
