...
- Agnostic `On*` registration fans adapter handlers onto each agent SDK `Adapter()` Chain (native Decode/Encode owned by `sdk/claude`, `sdk/copilot`, `sdk/cursor`); inbound typed `Map*` helpers map native payloads to unified events
