...
- **`internal/core`** — Host-neutral hook types and behavior: `HookContext`, `TemplateBindings`, `Command`, `HookHandler`, `HookHandlerResult` (see `hook_handler_result.go`), `HookHandlerFactory`.
- **`internal/run`** — The `run` wat subcommand as [core.Command] (`NewRunCommand`), flag parsing, and subprocess execution.
- **`internal/cursor`** — Cursor-specific hook JSON, per-event payloads, `HookHandlerFactory`, and the handler registry.
- **`internal/cursor/core`** (package `cursorcore`) — Shared Cursor hook stdin shape (`HookDataCommon`), common + event template bindings, default hook handler, and generic event-hook plumbing.
