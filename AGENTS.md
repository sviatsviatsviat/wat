...
- **`internal/run`** — The `run` wat subcommand as [core.Command] (`NewRunCommand`), flag parsing, argv templating for Cursor (`TemplateBindings` built from `cursorcore.CursorHookRunData[T]` on `HookContext.ParsedData`), and subprocess execution.
- **`internal/cursor`** — Cursor-specific hook JSON, per-event payloads, `HookHandlerFactory`, and the handler registry.
- **`internal/cursor/core`** (package `cursorcore`) — Shared Cursor hook stdin shape (`HookDataCommon`), generic parsed run payload (`CursorHookRunData[T]` with `EventSpecific *T`), `CursorHookHandler[T]`, and `HookHandlerBuilder` plumbing.
