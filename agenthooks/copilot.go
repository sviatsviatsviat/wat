...
const CopilotWarnExit = copilothook.WarnExit

// CopilotCodec implements Codec for GitHub Copilot hooks by adapting copilothook types.
//
...
func (c *CopilotCodec) Decode(raw []byte, eventHint string) (*Event, error) {
	native, err := copilothook.Decode(raw, copilothook.WithEvent(eventHint))
	if err != nil {
...
func (c *CopilotCodec) Encode(ev *Event, res Result) ([]byte, int, error) {
	...
	b, code, err := copilothook.Encode(copilotEncodeEventName(ev), out)
	if err != nil {
...
// AsCopilot re-decodes Event.Raw into a copilothook native event type.
func AsCopilot[T copilothook.Event](ev *Event) (T, error) {
	var zero T
	...
	native, err := copilothook.Decode(ev.Raw, copilothook.WithEvent(ev.Name))
	if err != nil {
		return zero, err
	}
	typed, ok := native.(T)
