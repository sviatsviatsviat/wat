...
func Encode(eventName string, out any) ([]byte, int, error) {
	...
	default:
		return nil, 0, fmt.Errorf("cursorhook: encode: unsupported output type %T", out)
	}
}
...
func validateEncodePair(eventName string, out any) error {
	...
	if !ok {
		return fmt.Errorf("cursorhook: encode: unsupported output type %T", out)
	}
	...
	return fmt.Errorf("cursorhook: encode: event %q incompatible with output type %T", eventName, out)
}
